...
const teachBot = async (api, threadID, messageID, senderID, teachText) => {
  ...
  try {
    ...
    const res = await axios.get(
      `${apiBase}/vixa/teach?ask=${encodeURIComponent(ask)}&ans=${encodeURIComponent(answerArray.join(","))}&uid=${senderID}`
    );
...
const talkWithBot = async (api, threadID, messageID, senderID, input) => {
  try {
    ...
    const res = await axios.get(
      `${apiBase}/vixa?text=${encodeURIComponent(input)}&uid=${senderID}&font=2`
    );
...
const botMsgInfo = async (api, threadID, messageID, senderID, input) => {
  try {
    ...
    const res = await axios.get(
      `${apiBase}/vixa/msg?ask=${encodeURIComponent(input)}&uid=${senderID}`
    );
...
const deleteMessage = async (api, threadID, messageID, senderID, input) => {
  try {
    ...
    let url = `${apiBase}/vixa/delete?text=${encodeURIComponent(text)}&uid=${senderID}`;
...
const editMessage = async (api, threadID, messageID, senderID, input) => {
  try {
    ...
    if (!isNaN(newAskOrIndex) && newAns) {
      ...
      const res = await axios.get(
        `${apiBase}/vixa/edit?ask=${encodeURIComponent(ask)}&index=${index}&newAns=${encodeURIComponent(newAns)}&uid=${senderID}`
      );
      ...
      const res = await axios.get(
        `${apiBase}/vixa/edit?ask=${encodeURIComponent(ask)}&newAsk=${encodeURIComponent(newAskOrIndex)}&uid=${senderID}`
      );
...
module.exports.onChat = async ({ api, event }) => {
  ...
  const cMessages = ["🎀 Hello,I'm **Vixa!**\n you can call me **Alia** !", "🎀 Hi there! I'm **Vixa** call me **Alia**", "Yes.!\nI'm Here only for you💖", "🎀 Hey! How can I help?💖"];
  const userInput = body.toLowerCase().trim();

  const keywords = ["sona", "vixa", "hello", "hi", "alia", "janu", "hlw"];
  ...
  if (keywords.some((keyword) => userInput.startsWith(keyword))) {
    ...
    if (isQuestion) {
      ...
      try {
        const res = await axios.get(
          `${await getAPIBase()}/vixa?text=${encodeURIComponent(question)}&uid=${senderID}&font=2`
        );
